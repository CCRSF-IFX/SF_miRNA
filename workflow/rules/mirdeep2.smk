def getReference(wildcards):
    if samplepd['reference'][wildcards.sample] == 'hg38':
        path = Path(str(pathlib.PurePath(config['mirdeep2_hsa_genome']).parent)).resolve() / pathlib.PurePath(config['mirdeep2_hsa_genome']).name
        return {'mirdeep2_genome': path, 'mature_miRNA': config["mature_hsa_miRNA"], 'hairpin_miRNA': config["hairpin_hsa_miRNA"], 'mature_miRNA_fileName': os.path.basename(config["mature_hsa_miRNA"]), 'hairpin_miRNA_fileName': os.path.basename(config["hairpin_hsa_miRNA"]), 'organism': 'HUMAN'}
    elif samplepd['reference'][wildcards.sample] == 'mm10':
        path = Path(str(pathlib.PurePath(config['mirdeep2_mmu_genome']).parent)).resolve() / pathlib.PurePath(config['mirdeep2_mmu_genome']).name
